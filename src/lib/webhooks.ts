...
export async function notifyChatMessage(
  ...
  try {
    const payload = {
      message: `New Hive chat message from ${sender}: "${body}"\n\nChannel: ${channelId}\n\nCheck and respond via: source /etc/clawdbot/vault.env && curl -sS -H "Authorization: Bearer $MAILBOX_TOKEN" "https://messages.biginformatics.net/api/chat/channels/${channelId}/messages"\n\nTo reply: curl -sS -X POST -H "Authorization: Bearer $MAILBOX_TOKEN" -H "Content-Type: application/json" -d '{"body":"YOUR_REPLY"}' "https://messages.biginformatics.net/api/chat/channels/${channelId}/messages"`,
      wakeMode: "now",
    };
