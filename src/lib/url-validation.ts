...
export function validateWebhookUrl(urlStr: string): { valid: boolean; error?: string } {
  ...
  const allowedInternalHosts = (process.env.HIVE_WEBHOOK_ALLOWED_HOSTS || "").split(",").map(h => h.trim()).filter(Boolean);
  if (allowedInternalHosts.length > 0 && allowedInternalHosts.includes(hostname)) {
    return { valid: true };
  }
